...
let db,currentId=null,currentObjectUrl=null,currentTexture=null;
...
const uniforms={projection:gl.getUniformLocation(program,"uProjection"),view:gl.getUniformLocation(program,"uView"),texture:gl.getUniformLocation(program,"uTexture")};
const attributes={position:gl.getAttribLocation(program,"aPosition"),uv:gl.getAttribLocation(program,"aUv")};
...
async function init(){
  ...
  renderGallery(items);
  if(items.length>0) await loadPanorama(items[0].id,{hideGallery:false});
  else{showGallery();showToast("Lade ein equirektangulares 360°-Panorama hoch.");}
  onResize();
  animate();
}
...
function createRenderer(){
  ...
  const context=canvas.getContext("webgl",{antialias:true,alpha:false,powerPreference:"high-performance"});
  if(!context){viewer.innerHTML='<div class="fatal-error">WebGL ist in diesem Browser oder auf diesem Gerät nicht verfügbar.</div>';throw new Error("WebGL not available");}
...
function bindEvents(){
  ...
  deleteCurrentButton.addEventListener("click",deleteCurrentPanorama);
...
async function deleteCurrentPanorama(){
  if(!currentId||!window.confirm("Aktuelles Panorama wirklich löschen?")) return;
  await deletePanorama(currentId);
  currentId=null;
  const items=await getAllPanoramas();
  renderGallery(items);
  if(items.length>0) await loadPanorama(items[0].id,{hideGallery:false});
  else{setPlaceholder();showGallery();showToast("Galerie ist leer.");}
}

async function clearGallery(){
  if(!window.confirm("Alle gespeicherten Panoramas löschen?")) return;
...
function onPointerDown(event){
  if(galleryPanel.classList.contains("open")) hideGallery();
  pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});
  viewer.setPointerCapture?.(event.pointerId);
  if(pointers.size===1){isDragging=true;lastPointerX=event.clientX;lastPointerY=event.clientY;viewer.classList.add("dragging");}
  if(pointers.size===2){isDragging=false;lastPinchDistance=getPinchDistance();}
}
...
function onPointerMove(event){
  ...
  if(pointers.size===2){
    ...
    if(lastPinchDistance>0) targetFov=clamp(targetFov-(nextDistance-lastPinchDistance)*0.08,35,100);
...
function onPointerUp(event){
  ...
  lastPinchDistance=0;
  if(pointers.size===1){const p=[...pointers.values()][0];isDragging=true;lastPointerX=p.x;lastPointerY=p.y;}
  else{isDragging=false;viewer.classList.remove("dragging");}
}
...
function onDragEnter(e){e.preventDefault();dropOverlay.classList.add("visible");}
function onDragLeave(e){if(e.clientX<=0||e.clientY<=0||e.clientX>=window.innerWidth||e.clientY>=window.innerHeight) dropOverlay.classList.remove("visible");}
async function onDrop(e){e.preventDefault();dropOverlay.classList.remove("visible");await saveFiles([...e.dataTransfer.files].filter(f=>f.type.startsWith("image/")));}
...
async function saveFiles(files){
  ...
  if(images.length===0){showToast("Keine unterstützten Bilddateien gefunden.");return;}
  let firstSavedId=null;
  for(const file of images){
    const record={id:crypto.randomUUID(),name:file.name||"panorama",type:file.type||"image/jpeg",size:file.size,createdAt:Date.now(),blob:file};
    await putPanorama(record);
    firstSavedId??=record.id;
  }
  const items=await getAllPanoramas();
  renderGallery(items);
  if(firstSavedId) await loadPanorama(firstSavedId,{hideGallery:true});
...
function setPlaceholder(){
  if(currentObjectUrl) URL.revokeObjectURL(currentObjectUrl);
  currentObjectUrl=null;currentId=null;
  if(currentTexture) gl.deleteTexture(currentTexture);
  currentTexture=createTextureFromCanvas(createPlaceholderCanvas());
...
function renderGallery(items){
  ...
  for(const item of items){
    ...
    button.className="thumb";button.type="button";button.dataset.id=item.id;button.title=item.name;
    const img=document.createElement("img");
    img.src=objectUrl;img.alt=item.name;img.loading="lazy";
    img.addEventListener("load",()=>URL.revokeObjectURL(objectUrl),{once:true});
    const info=document.createElement("span");info.className="info";
    const title=document.createElement("strong");title.textContent=item.name;
    const meta=document.createElement("span");meta.className="meta";meta.textContent=`${formatBytes(item.size)} · ${formatDate(item.createdAt)}`;
    info.append(title,meta);button.append(img,info);
...
function markActiveThumb(){for(const thumb of galleryGrid.querySelectorAll(".thumb")) thumb.classList.toggle("active",thumb.dataset.id===currentId);}
...
async function exportGallery(){
  ...
  const exportedItems=[];
  for(const item of items) exportedItems.push({id:item.id,name:item.name,type:item.type,size:item.size,createdAt:item.createdAt,dataUrl:await blobToDataUrl(item.blob)});
  const payload={app:"AI 360 VR Viewer",version:2,exportedAt:new Date().toISOString(),items:exportedItems};
  const blob=new Blob([JSON.stringify(payload)],{type:"application/json"});
  const url=URL.createObjectURL(blob);
  const link=document.createElement("a");
  link.href=url;link.download=`ai-360-gallery-${new Date().toISOString().slice(0,10)}.json`;link.click();
...
async function importGallery(file){
  try{
    ...
    let imported=0,firstImportedId=null;
    for(const item of payload.items){
      if(!item.dataUrl?.startsWith("data:image/")) continue;
      const blob=dataUrlToBlob(item.dataUrl);
      const id=crypto.randomUUID();
      await putPanorama({id,name:item.name||"imported-panorama",type:blob.type||item.type||"image/jpeg",size:blob.size,createdAt:Date.now()+imported,blob});
      firstImportedId??=id;imported++;
    }
    const items=await getAllPanoramas();
    renderGallery(items);
    if(firstImportedId) await loadPanorama(firstImportedId,{hideGallery:true});
    showToast(`${imported} Bild${imported===1?"":"er"} importiert.`);
  }catch{showToast("Import fehlgeschlagen. Erwartet wird ein Galerie-Export als JSON.");}
}

function showGallery(){galleryPanel.classList.add("open");galleryPanel.setAttribute("aria-hidden","false");galleryToggle.setAttribute("aria-label","Galerie schließen");}
function hideGallery(){galleryPanel.classList.remove("open");galleryPanel.setAttribute("aria-hidden","true");galleryToggle.setAttribute("aria-label","Galerie öffnen");}
function showToast(message){toast.textContent=message;toast.classList.add("visible");window.clearTimeout(showToast.timeout);showToast.timeout=window.setTimeout(()=>toast.classList.remove("visible"),2600);}

function openDatabase(){return new Promise((resolve,reject)=>{const request=indexedDB.open(DB_NAME,DB_VERSION);request.onupgradeneeded=()=>{const database=request.result;if(!database.objectStoreNames.contains(STORE_NAME)){const store=database.createObjectStore(STORE_NAME,{keyPath:"id"});store.createIndex("createdAt","createdAt");}};request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error);});}
function transaction(mode="readonly"){return db.transaction(STORE_NAME,mode).objectStore(STORE_NAME);}
function putPanorama(record){return new Promise((resolve,reject)=>{const request=transaction("readwrite").put(record);request.onsuccess=()=>resolve();request.onerror=()=>reject(request.error);});}
function getPanorama(id){return new Promise((resolve,reject)=>{const request=transaction().get(id);request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error);});}
function getAllPanoramas(){return new Promise((resolve,reject)=>{const request=transaction().getAll();request.onsuccess=()=>resolve(request.result.sort((a,b)=>b.createdAt-a.createdAt));request.onerror=()=>reject(request.error);});}
function deletePanorama(id){return new Promise((resolve,reject)=>{const request=transaction("readwrite").delete(id);request.onsuccess=()=>resolve();request.onerror=()=>reject(request.error);});}
function clearPanoramas(){return new Promise((resolve,reject)=>{const request=transaction("readwrite").clear();request.onsuccess=()=>resolve();request.onerror=()=>reject(request.error);});}
...
function createTextureFromCanvas(canvas){return createTextureFromImage(canvas);}
...
function createPlaceholderCanvas(){
  ...
  gradient.addColorStop(0,"#0f172a");gradient.addColorStop(0.5,"#111827");gradient.addColorStop(1,"#020617");
  context.fillStyle=gradient;context.fillRect(0,0,canvas.width,canvas.height);
  context.strokeStyle="rgba(103,232,249,0.18)";context.lineWidth=2;
  for(let x=0;x<=canvas.width;x+=128){context.beginPath();context.moveTo(x,0);context.lineTo(x,canvas.height);context.stroke();}
  for(let y=0;y<=canvas.height;y+=128){context.beginPath();context.moveTo(0,y);context.lineTo(canvas.width,y);context.stroke();}
  return canvas;
}
...
function makePerspective(fieldOfViewRadians,aspect,near,far){const f=Math.tan(Math.PI*0.5-0.5*fieldOfViewRadians),rangeInv=1/(near-far);return new Float32Array([f/aspect,0,0,0,0,f,0,0,0,0,(near+far)*rangeInv,-1,0,0,near*far*rangeInv*2,0]);}
function makeLookAt(cameraPosition,target,up){const zAxis=normalize(subtract(cameraPosition,target)),xAxis=normalize(cross(up,zAxis)),yAxis=cross(zAxis,xAxis);return new Float32Array([xAxis[0],yAxis[0],zAxis[0],0,xAxis[1],yAxis[1],zAxis[1],0,xAxis[2],yAxis[2],zAxis[2],0,-dot(xAxis,cameraPosition),-dot(yAxis,cameraPosition),-dot(zAxis,cameraPosition),1]);}
